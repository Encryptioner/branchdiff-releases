...
- **Then each section's Level 2 file drill-down**. Never wrap it in a collapsible `<details>` block — GitHub and Bitbucket PR comments render it as literal HTML tags, not a collapsible widget. Same floor and same latitude as Level 1 — prune and reorder for readability, never add or relabel beyond what the map's own edges say:
