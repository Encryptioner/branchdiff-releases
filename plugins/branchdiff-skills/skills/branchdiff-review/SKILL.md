...
Larger diffs append a machine-computed `BRANCHDIFF CHANGE MAP` block after the diff (Step 2) or after the summary table (`review context`), opening with **Level 0 — Summary** — always present — followed by up to three more levels. Those deeper levels are numbered by what THIS diff actually renders, not a fixed scheme: a skipped level is never counted, so on a diff with no whole-change overview the file drill-down prints as Level 1, not Level 2. Always read the level number straight off the map's own heading text rather than assuming one — never write "Level 2" in your comment because that is what it usually is.

**Level 0** is the part that is not a diagram: the diff's tier and per-area churn stats, plus — when the diff declares any — "What this change does": the endpoints the change registers, the CLI commands and flags it adds, the tables and columns it touches, the dependencies it pulls in or drops, exports it removed (where something outside the diff may break) and exports it merely relocated (a refactor, where nothing breaks), what its new tests assert in the author's own words, directories it moved, and which touched files the rest of the codebase leans on hardest. Every one of those is read out of the diff's own lines, so it names only what the change actually contains.

The next level, when the map draws one, is the whole-change overview — one diagram whose clusters are the PR's sections (areas wired together by imports), with dashed nodes for existing code consumed and dotted arrows for existing code consuming the change. A single section with no cross-area wiring earns no overview at all — that is not an omission to flag, it just means the level after it (the file drill-down) carries a lower number than usual. The file drill-down level, when file-level wiring exists, drills into each section at file level — every changed file styled by status (added/deleted/renamed), edges labeled with the new symbols flowing between files, dashed boxes for existing files or packages touched, and "removed" edges where the diff unwired an import. The last level, on large diffs only, tabulates the hottest files' new symbols with the author's own doc comments and who imports them.

You receive exactly one diagram format: mermaid, which renders on GitHub and in branchdiff's comment view, except on a session whose remote is Bitbucket, where mermaid doesn't render at all and the pre-rendered ASCII tree is the diagram instead — including on a bare `--refs` comparison with no linked PR, where mermaid is still the default. Its job is to hand a reviewer opening a large PR the orientation up front, top-down — the whole change first, then each part — so review time goes to substance instead of archaeology. Use it as your orientation, do not re-derive any of it, and never put a node, edge, area or file in your diagram that has no basis in it.

When a map is present, the review's general comment (Step 5) is layered exactly like the map:

- **Open with 1-3 sentences of intent** — what the PR does and why, in plain prose, before anything visual. Take those sentences FROM Level 0's "What this change does" facts when it carries any (the endpoints, the schema changes, the relocations it already named) rather than re-deriving them from the diff, and add only the *why*, which is the one thing it cannot compute. When Level 0 carries no such facts (a pure internal refactor), draw the sentences from the diff and PR description already in your context (Steps 2-3); never re-read files for this. Skip the sentences only when the diff is too mechanical to have an intent beyond its mechanics (a pure rename, a dependency bump).
- **Then the whole-change overview diagram, when the map drew one** — a reader who stops here still understands the change. Copying that block verbatim is always safe, and stopping there is fine (whichever format it included; mermaid on GitHub/branchdiff and ascii on Bitbucket when it shipped both).
- **Then each section's file drill-down**. Never wrap it in a collapsible `<details>` block — GitHub and Bitbucket PR comments render it as literal HTML tags, not a collapsible widget. Same floor and same latitude as the overview — prune and reorder for readability, never add or relabel beyond what the map's own edges say:
...
- **Symbol tables ride along when the map ships them** — copy as given; they are already the hottest files capped to the symbols that matter.
...
- **Repeat passes** — the map reports a prior pass count and New areas is empty → skip the diagram; the structure is unchanged and the existing one is still accurate. New areas → post the updated diagram(s) as this pass's general comment; the earlier diagram stays in its own pass thread, accurate for the pass it described.
- **Hybrid assist** — if exactly one wiring detail is unclear (e.g. what an edge actually carries) and the symbol tables do not already cover it, read that ONE file via `branchdiff agent file <path> --ref <b2> $SEL` and add a short prose note after the copied diagram. One file only, and it can inform a label edit — never a new node or edge.
...
- Change map present → that general comment is layered like the map: intent prose taken from Level 0, the whole-change overview diagram when the map drew one, then per-section file drill-downs (see Step 3).
