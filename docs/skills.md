...
3. **Bump versions** — the plugin/marketplace version tracks the `branchdiff` CLI version it was rendered from (e.g. skill content rendered from CLI `v2.0.0` → plugin version `2.0.0`), so a user can tell which CLI release a skill matches at a glance:
