...
`history` lists commits from the start of the repo up to any ref or range, with search and a **First parent**/**Merges**/**Graph** toggle set — all three stay highlighted while active and compose together, so turning one on never silently switches another off; click a commit for the full diff view. **First parent** walks only the ref's own line, skipping over whatever a merge pulled in from elsewhere — useful on a range (`main..feature`) where the feature branch has synced from a shared branch and the plain log would otherwise be flooded with commits that aren't really this range's own work. The header shows how many commits match, **Load all** fetches the rest of the list in one go, and hovering a commit reveals copy-hash and open-in-new-tab buttons.
...
5. Edit the title or add a description, then click **Create Pull Request**. The description field has a **Write/Preview** toggle (preview renders the markdown exactly as branchdiff's comment view does) and an **Insert** row with **Commit history** and **Change map** buttons that splice the branch's commits or the current [change map](#the-change-map-how-this-change-works) in at the caret as editable markdown — mermaid on GitHub, an uploaded image on Bitbucket for the map; re-clicking either button updates its existing block instead of duplicating it
