...
`history` lists commits from the start of the repo up to any ref or range, with search and a **First parent**/**Merges**/**Graph** toggle set — all three stay highlighted while active and compose together, so turning one on never silently switches another off; click a commit for the full diff view. Both toggles start **off** — the default list is every reachable commit, merges included. **First parent** walks only the ref's own line, skipping over whatever a merge pulled in from elsewhere — useful on a range (`main..feature`) where the feature branch has synced from a shared branch and the plain log would otherwise be flooded with commits that aren't really this range's own work. The header shows how many commits match, next to a copy button that copies every currently loaded commit (hash + message, one per line) to the clipboard — a checkmark confirms the copy; it copies what's actually been paged in, not the full match count, so **Load all** first for the complete list. **Load all** fetches the rest of the list in one go, and hovering a commit reveals copy-hash and open-in-new-tab buttons.
...
Diffs big enough to need orientation — **3+ files or 80+ changed lines** — come with a **change map**: a machine-computed block appended to `branchdiff agent diff` output and inserted after the summary table in `review context` and `review run`.

It opens with **what this change does** — a handful of lines naming the endpoints it registers, the CLI commands and flags it adds, the tables and columns it touches, the dependencies it pulls in or drops, the public exports it removes (which is where something outside the diff may break), the exports it merely relocated (which is where nothing breaks), the behaviour its new tests assert in the author's own words, files and directories it moves, and which of the touched files the rest of the codebase depends on most heavily. Each of those is read straight out of the diff — a route counts as a route because a line registers one — so the summary names only what the change actually contains, and says nothing at all when a change declares none of them. That is the part a reader needs before deciding where to look, and the part an import graph structurally cannot answer: routes, columns and dependencies are declarations, not edges.

**A refactor reads as a refactor.** Pulling a helper into a new module and re-exporting it from its old home is the most ordinary change there is, and calling that "public API removed, callers may break" cries wolf on precisely the change that broke nothing. Three cases are told apart: a symbol still exported from its original file never left and isn't mentioned; a symbol whose declaration reappears in a different file in the same diff is reported as a move naming both ends; only a symbol that is nowhere to be found afterwards is a removal, and only that one carries the warning. The same holds a level up — a deleted file and an added file sharing a name are reported as one file moved, which is what a move combined with a rewrite looks like once git scores the contents too far apart to call it a rename.

Under it sits the structural picture: which areas moved and by how much, which are wired together by imports, whether the PR is one coherent change or several unrelated ones (**sections**), which areas are entirely new, and — once there has been one — how many review passes the session has already had. Both diagram levels share a single legend, and the notes beside them (areas elided for space, test files left out of a diagram, how much of the tree the import scan managed to read) sit in italics: there for anyone who needs them, out of the way of anyone who doesn't. For every section with real import wiring, the map already includes a ready-made diagram — a section with no wiring (an isolated doc, a lone config file) gets none, since a diagram with no edges would say nothing a one-line description doesn't already say. The whole-change overview is skipped on the same reasoning when it would draw a single cluster whose every arrow points at existing code the change consumes: that is one box and a fan of dependencies, which the area table above already lists and the per-file drill-down below already shows in more detail. A one-line summary names the sections instead, including the one thing a table cannot say — whether this is one change or several with no import path between them. When an edge crosses into code the diff itself added (a new function, class, or export the other side actually imports), the edge is labeled with those names — the diagram reads as "this PR added X, which Y now calls", not just "these two areas happen to be connected". An import count rides along only when a file is imported more than once (`×3`); a single import is what an arrow already means. When that new symbol carries its own doc comment, its node goes further still: the box itself folds in a short line of what the symbol actually does, drawn from the comment's first sentence — the diagram shows behavior, not just structure, wherever the diff already wrote it down. A symbol with no comment (common — not everything gets documented) just keeps its plain name; nothing is invented to fill the gap — instead the map names that gap explicitly, so the AI reviewer can fill it cheaply from a file it's already reading for the review, rather than never at all or at the cost of an extra read.

The AI reviewer opens its **"How this change works" general comment** with a short intent summary — taken from that first section rather than worked out again from the diff — followed by those diagrams, copied as-is by default. It receives exactly one diagram format: mermaid, which renders on GitHub and in branchdiff's comment view, except on a session whose remote is Bitbucket, where mermaid doesn't render at all and the pre-rendered ASCII tree is the diagram. The browser modal is the one place both formats ship, because a reader there can toggle between them. Copying verbatim is always safe, but the AI has real latitude beyond that floor — sharpening a label, pruning a genuinely redundant edge, or filling in a short description for a node the map flagged as undocumented (see below) but which the AI already read this pass. The one line it won't cross: every node and edge that appears must trace back to a real one from the map — nothing invented. Small diffs get no map and owe no diagram, and a repeat pass with no new areas skips re-posting one.
...
The map can also be dropped straight into a PR description: the [create-PR](#creating-pull-requests-from-the-ui) and [edit-PR](#managing-prs-lifecycle-actions) dialogs' description field has an **Insert** row with a **Change map** button that appends the current map to the end of the description as ordinary editable mermaid markdown — the end, never the caret: an inserted block is a self-contained section (its own heading and closing rule) and must not land mid-sentence inside your own prose. The markdown is the same on both platforms, so the Write/Preview toggle renders the real diagram either way while you're still drafting. Bitbucket can't render mermaid on its own page, so on Bitbucket every diagram (the whole-change overview and each section's file drill-down) is converted to an uploaded image automatically at save time — never on insert or while just previewing, so nothing is written to the repo until you actually create or save the PR. Re-clicking after the diff has moved on doesn't duplicate the block — it finds the one already in the description and replaces it, so refreshing a stale map (or the **Commit history** button next to it, same row) is one click, not a manual delete-and-reinsert. The button reads **Update change map** once a block is already present, and a small **×** button next to it removes the block outright. Each inserted block opens with its own `### Change map` / `### Commit history` heading and closes with a `---` rule, so a description carrying both (or either alongside your own prose) reads as clearly bounded sections, not one undifferentiated wall of markdown. A **Clear** button next to the Write/Preview toggle, above the description field, empties the whole field in one click — useful for starting a description over from scratch rather than deleting each inserted block one at a time. Any image in a rendered description preview — an uploaded Bitbucket map included — opens fullscreen on click, with the same zoom in/out/reset controls the change-map diagram dialog has. In either fullscreen viewer — image or diagram — once the content is wider than the dialog the cursor turns into a hand: grab it and drag to pan, which is the quickest way around a zoomed-in diagram or a wide change map. Ctrl/⌘+scroll, or a trackpad pinch, zooms in and out centered on the cursor; plain scroll still scrolls.

<details><summary>Technical breakdown</summary>

- The block's marker is its own visible `### Change map` / `### Commit history` heading down to its closing `---` rule — no separate wrapper. Bitbucket renders HTML comments as literal text rather than hiding them the way GitHub does, so the heading+rule shape doubles as both the reader-visible section boundary and the greppable span `findInsertBlock` scans for.
- The opening "what this change does" section is scanned out of the diff's own hunk lines — no file reads, no git calls, no network — so it costs a single pass over text already in memory and survives even when the import scan can't run. Coverage is two-tiered by design: dependencies (any manifest file), schema (SQL keywords plus the ORM and migration DSLs — Prisma, Django, Rails, Ecto, Knex, SQLAlchemy, Mongoose), moves (git's own rename status, plus a deleted/added pair sharing a basename) and test titles are decided from paths and syntax no language owns, so they work in a repo written in anything; endpoints and CLI surface are per-framework patterns layered on top, and a stack with no matching pattern simply contributes nothing to those lines rather than guessing. Every list is capped, and nothing is ever invented: each name printed is a substring of a line the diff actually contains. Lines that are only comments are skipped throughout — a doc comment naming an example route is documentation, not something the program serves.
- Three rules keep the endpoint list honest. A route is only reported when a line carries **both** a route-declaring token and a path-shaped literal, which keeps an outbound `axios.get('/users')` and a `/static/logo.svg` handler out of a list of API routes. A method-level annotation is joined to the base path its class declares (`@Get(':id')` under `@Controller('users')`, `@GetMapping("/summary")` under a class `@RequestMapping`, `[HttpGet("{id}")]` under `[Route(...)]`) — alone those read as paths no application serves, and the mount line itself is not listed because the methods beneath it already carry it. And a newly added file under a file-per-route convention has its URL read from its path (Next.js `app/**/route.ts` and `pages/api/**`, SvelteKit `+server.ts`, Nuxt/Nitro `server/api/**`), with route groups dropped, dynamic segments normalized (`[id]` → `:id`, `[...rest]` → `*`) and a method suffix (`orders.post.ts`) taken as the verb — no line in those files ever names the URL, so a line scan structurally cannot see them. A hand-rolled router matching a prefix or a regex reports the literal part with an ellipsis, rather than pretending the prefix is the whole route.
- Removed exports are reconciled against every export the same diff adds before anything is called a removal. A name still exported from its own file — through a re-export (`export { x } from './y'`, `export *`), python's `__all__` or rust's `pub use a::{x}` — never left. A name whose declaration reappears as an export of another file moved, and is reported as a relocation naming both ends. Only what is nowhere to be found is a removal, and only that line warns that callers outside the diff may break.
- Blast radius reuses the consumer scan the wiring already runs: for each changed file, how many unchanged files import it, ranked, with a note when the file changed and its test file did not. It's a lower bound — the consumer read is capped — and it's simply absent when the wiring scan didn't run.
- The hot-file symbol table drops any row that would carry neither a description nor a consumer outside its own tests: a bare name with two em dashes says nothing the export list above it hasn't already said. A file whose every new symbol is that shape gets no table.
...
- Every insert drops the session-bookkeeping `Tier:` line (which also carries the session's pass count); it informs a review pass, not a PR reader. The spliced-in block (on either platform) opens with a `### Change map` heading and closes with a `---` rule — both already meant to be visible to a PR reader, and together they're also the marker a re-click of **Insert change map**/**Update change map** scans for to find and replace exactly that span instead of appending a second copy (`findInsertBlock`/`upsertBlock`, `change-map.ts`). No separate invisible wrapper: an HTML comment renders as literal text on Bitbucket, so it isn't invisible everywhere, and the visible heading is a marker Bitbucket can't mangle. The button's own label is just a live lookup for that heading in the current description text, not separately tracked state — and the same lookup drives the **×** remove button's visibility.
...
- **Commit history** is the row's other button, identical marker/heading/update-in-place mechanics but a different source: it's a fresh `git log --first-parent b1..b2` (the local session's compared branches, whichever the dialog is scoped to), not the change map — under its own `### Commit history` heading, a plain `shortHash` + message list. `--first-parent` walks only the branch's own line rather than a plain two-dot log, so a branch that periodically merges a shared integration branch in doesn't drag that branch's commits into the description — this insert is always first-parent, with no toggle (unlike the standalone history view, which defaults to the full walk and lets the reader switch). Merge commits stay in the list, tagged `_(merge)_`. The list is capped at 100 rows — a capped list gets `(N of total commits)` appended right onto the `### Commit history` heading, so it never silently reads as the whole story — and any single subject line past 120 characters is truncated with `…` so one runaway line can't blow up the list. It fetches on every click, so a stale insert (new commits landed since the last click) is fixed the same way a stale change map is — click the button again. No mermaid/image handling needed here; the list renders identically on both forges.
...
The prompt handed to the AI on stdin is also written to a private temp file, with its path exported as `BRANCHDIFF_CONTEXT_FILE` and named in lines at the very top and the very bottom of the prompt itself. A long review can push the AI's own context into compaction, which evicts the piped prompt from its window; with the file, recovering the full context (diff, change map, instructions — verbatim) is one read — re-read that file FIRST, before anything else — instead of reconstructing it from the AI tool's transcript, which is slow enough to time out a pass outright. The recovery must not rebuild the context by curling the branchdiff server, driving a browser or MCP tool, or re-running the session commands: those detours burn the pass budget and time out the review while producing less than the file already holds. The temp dir is named after the run's label — the PR number under `auto`, or the reviewed branch/ref for a local session (`working tree` when there is no ref) — plus the process id, so several reviews running in parallel (an `auto` cycle over multiple PRs, or several local branch reviews at once) each get a dir you can tell apart at a glance — and the dir is deleted as soon as the pass finishes, so a watch-mode `auto` re-reviewing the same PR every cycle always reads its own pass's file, never an earlier pass's leftover.
...
5. Edit the title or add a description, then click **Create Pull Request**. The description field has a **Write/Preview** toggle (preview renders the markdown exactly as branchdiff's comment view does — click any image in it to open fullscreen with zoom) and an **Insert** row with **Commit history** and **Change map** buttons that append the branch's commits or the current [change map](#the-change-map-how-this-change-works) to the end of the description as editable markdown — mermaid on GitHub, an uploaded image on Bitbucket for the map; re-clicking either button updates its existing block instead of duplicating it
...
Both pages carry a **User Guide** / **Changelog** link pair in the header — each opens the hosted copy on [`branchdiff-releases`](https://encryptioner.github.io/branchdiff-releases), which always reflects the latest published version, unlike the pages above which reflect the version you have installed.
...
- **Where "latest" comes from:** the npm registry. A release publishes to npm first and the other channels (Homebrew, pip, Scoop, apt, standalone binary) follow via CI, so on those channels the badge can appear shortly before `brew upgrade`/`apt install` can actually serve the version. The badge means "a new release exists".
- **How the browser update runs:** the server spawns `branchdiff update --pm <channel> --yes` and streams its output to the dialog. The dialog's two-step confirm is the confirmation layer, which is why the spawned command passes `--yes`.
- **Channels that need a terminal:** `apt` (sudo password) and a standalone binary installed to a sudo-owned path (`/usr/…`, `/opt/…`) can't be updated from the browser — the dialog shows the command with a Copy button instead of a Run button.
- **Offline is normal:** if the registry can't be reached, no badge appears and the dialog says so; nothing errors.
