...
> **Renamed or wrapper binary — e.g. you invoke `claude` as `claude-account1`.** `--tool` only accepts the six preset names, so `--tool claude-account1` errors out naming the known list. Hand it your binary with `--exec`, and if the binary is a claude-family tool whose skill-mode auto-approve flag you still want added, name the family with `--tool` too — the family is looked up for the flag even though `--exec` supplies the command:
> ```bash
...
> ```
> Under cron, the **env-prefix** and **wrapper script** forms described above (in "Using your own account, a wrapper, or any custom tool") survive unchanged — the `--exec` value is baked verbatim into the schedule, so nothing cron-specific to set up. One caveat per form:
...
>
> A bare shell alias resolves to nothing under cron (non-interactive `sh -c`, per above) — the two forms are what replaces it. Full env story at [Cron doesn't inherit your shell's `PATH` or env vars](#cron-doesnt-inherit-your-shells-path-or-env-vars).
...
- `cron add` prints exactly what it just scheduled — on Linux the two start/end crontab lines (the same ones `crontab -l` would show) plus the path of the generated script (below); on macOS the two LaunchAgent plist paths and their per-fire logs — right under the "Added cron schedule" confirmation, so you can see exactly what's scheduled without a separate lookup.
- **The full command lives in a generated script, not the crontab line itself.** Every scheduled flag/value, the forwarded env (`PATH`, `--notify`'s desktop vars), and the login-shell wrap all go into `~/.branchdiff/cron-scripts/<cronId>.sh` (macOS: the same path, referenced from the START LaunchAgent's plist) — the crontab START line itself is just `<cron> /bin/sh '<that script>'`, a small constant length no matter how many flags the schedule carries or how long `--repo-paths`/`--dest`/`--source`/`--prompt` are. That keeps a flag-heavy `cron add` — a long `--repo-paths` list, wide `--dest`/`--source` glob sets, a free-text `--prompt` — safely clear of the OS's crontab line-length cap (commonly ~1024 bytes), which is otherwise the limiting factor for how much a single scheduled job can carry. `cron remove` deletes the script along with the crontab/plist entries; nothing to clean up by hand.
...
- **A line break in a scheduled value is flattened, not written raw.** Crontab entries are newline-delimited, so a raw `\n`/`\r` inside any value (most likely a free-text `--prompt`) would split the generated line in two, leaving the first half on an unclosed quote — cron would run it anyway and fail with a `sh` syntax error at every single fire, with nothing to notice by (same silent-failure shape as the cwd mismatch above: no toast, no log, no session entry). Every value `cron add` writes is quoted through the same function, and that function replaces any line break with a space before quoting — so this can't happen regardless of which flag carries the newline. `cron add` still warns when it does this, naming the affected value, so you know your text was altered.
- **`--notify` under cron just works — no env vars to add yourself.** `cron add` copies `PATH` plus `DISPLAY`/`DBUS_SESSION_BUS_ADDRESS` from the shell you ran `add` in straight into that schedule's generated script (whichever of those are actually set — it prints which). Run `auto cron add ...` with `--notify` from your normal desktop session (not `sudo`, not a headless SSH session with no `DISPLAY`) and the toast works exactly like a foreground `--notify` run: on start/done/push/failure, at whatever hour the schedule fires, whether or not you're at the keyboard. Skip `--notify` on a headless server — there's no desktop session for a toast to reach regardless of what's forwarded.
- `auto stop --cron-id <cronId>` (by schedule) resolves to the same stop logic as `auto stop <sessionId>` (by session) — and finds a schedule's live session even when that session's registry entry is missing, as long as its per-repo lock is still held (same reconciled lookup `auto stopall` uses). A foreground/unregistered session stopped this way, or by `cron remove`/`removeall` below, logs `Stopped — requested from branchdiff auto stop --cron-id.`/`Stopped — requested from branchdiff auto cron remove.` in its own terminal, same pattern as the pid-stop and Stats-page messages above.
- `auto cron removeall` removes every schedule in one pass — real crontab/launchd entries (stopping each one's live session along the way) plus any orphaned generated script under `~/.branchdiff/cron-scripts/` left behind by a hand-removed scheduler entry. One line per schedule removed, plus a summary count; a failure on one schedule never skips the rest — and a schedule whose live session couldn't be confirmed stopped still counts as removed, since the crontab/script/plist entry is already gone by that point. Nothing configured is a clean, non-error message.
- Requires `--review` (same class of refusal as `--detach`). On Linux it needs a `crontab` binary and refuses cleanly on a platform without one, warning (doesn't block) if the `cron`/`crond` daemon doesn't appear to be running: the entry is written correctly, but nothing fires until the service is up. On macOS it uses the always-present `launchctl`/LaunchAgent substrate instead, so there's no daemon-to-check — the agents are loaded at `add` time and fire from there.
- **On macOS, schedules run via launchd — no Full Disk Access needed.** A user crontab job on macOS needs Full Disk Access since Catalina, or cron forks the job and kills it before exec with no error anywhere — the schedule looks permanently idle even though the daemon is up. So on macOS, `cron add` writes a per-user LaunchAgent (loaded with `launchctl`, reloaded automatically at every login so it survives reboots) instead of a crontab entry; a user LaunchAgent runs in your own login context and execs freely, so it fires with no grant and no manual macOS setup step. The `--start`/`--end` cron-expr vocabulary is unchanged — launchd's `StartCalendarInterval` takes the same fields (`*` = every, a single value = once, a range/list/step = each in the set; weekday 0/7 = Sunday) — and `cron list`/`remove` find the schedule the same way. The two plist files (`<cronId>.start`/`.end`) live in `~/Library/LaunchAgents`, per-fire logs in `~/.branchdiff/launchd/`, and `cron add` prints their paths. Linux keeps using `cron`/`crond` unchanged.
- **No catch-up.** Both schedule entries fire only at the exact wall-clock minute they match, only while the scheduler is alive (`cron`/`crond` on Linux, `launchd` on macOS) — a missed minute (scheduler was down, machine was off/asleep) is simply skipped, never queued or replayed once things come back. Adding the schedule after today's `--start` time already passed means today's window doesn't run; the next fire is whichever entry's expression next matches (typically tomorrow's `--start`). On an always-on Ubuntu server this is rarely an issue — `cron` ships enabled as a systemd service and resumes on its own after a reboot — but confirm it with `systemctl is-enabled cron` (enable with `sudo systemctl enable --now cron` if not); on macOS the LaunchAgents reload at every login, so a reboot recovers as soon as you log back in. There's no branchdiff-side mechanism to re-arm or backfill a window missed while the machine was down; if a reboot happens mid-window, the detached loop dies with it and stays down until the next scheduled `--start`.
...
- Unix only (launchd on macOS, `cron`/`crond` on Linux) — `--detach` itself has no such constraint.
...
- **Quick commands** — three collapsible sections (`branchdiff list`, `branchdiff auto list`, `branchdiff auto cron list`) that fetch live data on expand from dedicated endpoints (`/api/instances`, `/api/auto-sessions`, `/api/cron-schedules`), grouped by repo, and let you act on any entry directly: **Kill** an instance, **Attach** (copies the log-tail command to your clipboard) or **Stop** a running auto session, and **Remove** a cron schedule (also stopping its live session, if one is running). Every collapsible section — these three plus **Configs** below — has a **Refresh** button beside its Copy button to reload on demand if a list has gone stale while it stayed open. A **refresh icon at the top of the dashboard** reloads the whole page at once — the usage-stats query behind every section together with whichever of these live sections you currently have expanded — and spins while it works, toasting success or naming any section that failed to refresh. Every action button's tooltip names the exact CLI command it performs. A foreground (non-detached) auto session shows only **Stop** (no **Attach** — it has no log file to tail); stopping it this way prints `Stopped — requested from the Stats page.` in the terminal it's running in, so whoever's sitting at that terminal knows it wasn't their own Ctrl-C. An auto session found only through its live per-repo lock (no matching registry entry) is flagged **unregistered** — informational only, it stops exactly like any other row.
- **Bulk actions.** Each of the three sections above gets a page-level button once expanded with at least one entry — **Kill all** (Instances, same as `branchdiff killall`), **Stop all** (Auto sessions, same as `branchdiff auto stopall`), **Remove all** (Cron schedules, same as `branchdiff auto cron removeall`) — using the same click-to-arm, click-again-to-confirm pattern as every per-row action. Firing one disables every button (bulk and per-row) in that section until it resolves, then toasts a count of how many succeeded. The Instances section also badges the row whose port matches the one you're viewing the Stats page from as **this session** — informational only; **Kill** (single or **Kill all**) still works on it exactly like any other row, so forcing a restart of your own server is one click away, not blocked.
...
**Precedence** — a flag you pass on the CLI always wins. For the root command, `.branchdiff.json` in the directory you ran branchdiff from beats `~/.branchdiff/config.json`. For `auto`, every key falls into exactly one of three tiers, and the tier is what determines which config files it can come from:

- **Launch-wide** — resolved once, from the directory you ran `auto` from, *before* any repo is scanned (`repoPaths` is what determines the repo list in the first place, so it can't itself come from a per-repo file). Order: CLI flag → that directory's `.branchdiff.json` → global config → built-in default. `yes` is the one field in this tier **never** read from a target repo's own `.branchdiff.json` — its consent decision is made before any individual (possibly untrusted, discovered) repo's own config is ever read, so a discovered repo can't self-approve its own inclusion.
- **Per-repo** — resolved once *per target repo*, so a multi-repo run can give each repo its own size gates, skill, notify setting, and so on. Order: CLI flag → that repo's own `.branchdiff.json` → the launch directory's `.branchdiff.json` → global config → built-in default.
- **Exec-only** (`exec`, `tool`) — resolved **only** from `~/.branchdiff/config.json` or a CLI flag, never from any `.branchdiff.json` (launch-dir or per-repo). A `.branchdiff.json` can arrive via `git clone`, and `exec` runs a real shell command, so honoring it from a file you didn't author yourself would let a committed config run automatically the moment anyone clones the repo and runs `auto`.

| Tier | Fields | Can be set from |
|---|---|---|
| Launch-wide | `repoPaths`, `repoConcurrency`, `keepServers`, `watch`, `forceSession`, `detach` | CLI, launch-dir `.branchdiff.json`, global `~/.branchdiff/config.json` |
| Launch-wide (global/launch-dir only, never per-repo) | `yes` | CLI, launch-dir `.branchdiff.json`, global config — **not** a target repo's own `.branchdiff.json` |
...
| Exec-only | `exec`, `tool` | CLI, global config only — never any `.branchdiff.json` |

`branchdiff auto cron add` resolves `repoPaths` at the same launch-wide tier and in the same order (CLI → the directory you ran `cron add` from → global config) — so a schedule added with no `--repo-paths` at all still needs *some* tier to supply it, or `cron add` refuses the same way a bare `auto` run would from a directory with no repo and no children. Since the crontab job itself always runs with `cwd = $HOME` (see "Unattended, on a headless server" below), a launch-dir `.branchdiff.json` set anywhere other than `$HOME` won't be read again at fire time — put a durable `repoPaths` in `~/.branchdiff/config.json` (global) if you rely on config rather than `--repo-paths` for a scheduled run.
...
`branchdiff config sample --full` writes every valid key that has a fixed default, grouped under its correct `defaults`/`auto` section — useful when you want the exact spelling and section of an arbitrary key without cross-referencing the tables above by hand. A key whose real "unset" behavior has no single fixed value (e.g. `maxFiles`, `base`, `skillName` — see the table above and `CONTEXT_DEPENDENT_DEFAULTS`) is never guessed at with a placeholder; instead it's named in a note printed after the file is written, grouped the same way. `exec`/`tool` only become eligible keys with `--full --global`, matching the exec-only tier's own restriction.

`branchdiff config --json` emits the same resolution as machine-readable JSON, and `--dir <path>` resolves against an arbitrary directory instead of the one you're standing in — useful for checking what a projects folder would resolve to without `cd`-ing there first. Given a directory that isn't itself a repo, both the text and `--json` output expand to every repo directly under it (one block each), the same depth-1 rule `auto`'s own discovery uses — so `branchdiff config --dir ~/projects` shows exactly what each `~/projects/<repo>` would resolve to if `auto` were launched from `~/projects`.

A bare key name can exist in more than one section with a different meaning — `detach` is both a root-command flag (`defaults`) and a whole-run `auto` flag (`auto` launch tier); `worktree` is both a root-command flag (check out a PR URL into `.worktrees/pr-<n>`) and an `auto` per-repo flag (same idea, but for `auto`'s own PR checkout). The CLI and the Stats "Configs" section below always show each key under its own section heading (`defaults` vs `auto — …`) rather than as one flat list, so the two are never confused for each other.

**Stats dashboard — Configs section.** The same resolution is also browsable at `branchdiff stats` → **Configs** (after Cron schedules): pick a launch directory (defaults to the repo the dashboard server is running in) and expand **Global** / **Launch dir** / each repo found under it to see its raw file content plus, per repo, the fully-resolved per-key table with an override highlight wherever that repo's own file won. Copy a file's path or content, or open it in your default editor, straight from the tree. Global, Launch dir, and each repo block show **create** instead once you expand and find no file there — it writes that file seeded with the config currently in effect for that tier (skipping keys with no single fixed value, like `repoPaths`) and opens it in your default editor, so you start from a snapshot of what already applies instead of a blank file. The ⓘ button explains the same precedence rules as above — including the honest caveat that the launch-dir tier is wherever you point it, not a fixed parent folder, since branchdiff has no ancestor-directory config cascade.
...
A single `node_modules/better-sqlite3` holds one compiled binding at a time; it cannot serve two Node majors simultaneously. If you run branchdiff under `nvm use 24` sometimes and plain/default `node` (say, `nvm`'s default alias, often an older LTS) other times — including a `link`-ed dev checkout, where every Node version's global bin resolves to the *same* physical install — whichever Node version you last rebuilt the binding for works, and every other version breaks with the ABI-mismatch error above, even though `require('better-sqlite3')` alone can't tell (see `branchdiff doctor` above).

This matters most for `branchdiff auto cron add`: the START job's generated script (`~/.branchdiff/cron-scripts/<cronId>.sh`) bakes in `process.execPath` and `process.argv[1]` — the *exact* Node binary and branchdiff script that ran the `add` command — not a bare `branchdiff` resolved from `cron`'s PATH at fire time. That's deliberate: `cron` never sources `.bashrc`/`.nvmrc`/`nvm.sh` (see the next entry), so a bare `branchdiff` or an `nvm use <n> && branchdiff ...` line would need `nvm` fully initialized on every single fire — more moving parts, not fewer, and with `nvm.sh` itself as a new thing that can go wrong. Two consequences:
- **Changing your shell's default Node version doesn't move an already-scheduled job.** `nvm alias default <n>` has zero effect on a schedule that already exists — check what's actually pinned with `cat ~/.branchdiff/cron-scripts/<cronId>.sh | grep exec` (`branchdiff auto cron list` gives you the `cronId`; the script prints the exact node/branchdiff paths in plain text), and `auto cron remove --id <id>` + re-`add` under the Node version you want if it's wrong.
...
`auto cron add` handles both, differently depending on whether the value ever changes. `cron` never sources `.bashrc`/`.nvmrc` and starts every job with a short built-in `PATH` (typically just `/usr/bin:/bin`), so a bare `git`/`gh` call, an AI CLI installed to a user-local directory (`claude` in `~/.local/bin`, `opencode`/`gemini`/`cursor`/`codex` wherever their installer put them, npm's global bin), Bitbucket's env-var credentials (`BITBUCKET_USERNAME`/`BITBUCKET_API_TOKEN`), GitHub's (`GH_TOKEN`/`GITHUB_TOKEN`), or `--notify`'s desktop-session vars (`DISPLAY`/`DBUS_SESSION_BUS_ADDRESS`) can all resolve to nothing under cron even though they work fine when you run `branchdiff` yourself. `auto cron add` handles all of these with nothing to hand-edit in `crontab -e`, but not identically:

- **`PATH`, `DISPLAY`, `DBUS_SESSION_BUS_ADDRESS`** are copied once, as a snapshot, straight into that schedule's generated script from the shell running `add` (it prints which ones it copied) — fine for these, since none of them rotate.
- **Forge auth** (`BITBUCKET_USERNAME`/`BITBUCKET_API_TOKEN`/`GH_TOKEN`/`GITHUB_TOKEN`/`GH_HOST`/`GH_ENTERPRISE_TOKEN`) is never baked in as a snapshot — a token rotated after `cron add` would otherwise sit stale for the schedule's entire lifetime. Instead, the scheduled job's exec runs inside your own login shell, which sources your profile fresh on every single fire, so it always sees whatever these vars currently are wherever you actually export them (`.bashrc`, `.zshrc`, `.profile`, ...). A long-running `--watch` loop is one process across many cycles, so it can't get a fresh shell per cycle — it re-reads these same vars from your login shell at the top of every cycle instead, so a token you rotate mid-window is picked up by the very next cycle, not just the next `cron add`.

A team using only GitHub, only Bitbucket, or no `--notify` simply has fewer of the snapshot vars set, so fewer get copied — no warning either way; and forge auth just works as long as it's exported somewhere your login shell sources, same as running `branchdiff` yourself.

This covers the vars branchdiff itself needs to reach a forge or a desktop session. A custom `--exec` command's own env vars (an API key the tool reads itself, say) ride the same login-shell wrap: `export` one in `~/.zshrc`/`~/.bashrc` and the scheduled job sees it at every fire — no crontab edit. (A long-running `--watch` loop is one process across many cycles, so a key you rotate mid-window is picked up at the next fire, not the next cycle — restart the session if you need it sooner.) The alternative is a plain `VAR=value` line at the top of the crontab (`crontab -e` — it applies to every job in that file):
```
MY_TOOL_API_KEY=your-key
```
A schedule already running when you add one won't pick it up until its next fire: its already-spawned `--watch` loop keeps the environment it started with for its whole lifetime, forge auth (which refreshes every cycle, per above) aside — see **Covering today's gap** above for restarting it now instead of waiting.

If you write the crontab line yourself instead of using `auto cron add` — which already pins branchdiff's exact path into the scheduled job, copies the snapshot vars above, and wraps the exec in your login shell for live forge auth — none of this happens automatically; add `PATH`/`--notify` vars by hand, wrap your own command in `$SHELL -ilc '...'` (or just `export` forge auth directly on the crontab line) for live-resolved auth, plus `branchdiff` itself needs to be on that `PATH` (this prints its directory):
```bash
