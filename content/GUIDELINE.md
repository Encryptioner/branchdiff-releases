...
**No PATH setup under cron, systemd, or CI.** `auto` hands its own install location to the AI reviewer it spawns, so the reviewer resolves `branchdiff` by name even when the launcher's `PATH` is bare — cron's default, a systemd unit, a CI runner all work without you adding `branchdiff` to the launcher's `PATH`. `auto cron add` already invokes branchdiff by absolute path, so the schedule starts regardless; the reviewer it then launches is covered too.
...
- `cron add` prints exactly what it just scheduled — on Linux the two start/end crontab lines (the same ones `crontab -l` would show); on macOS the two LaunchAgent plist paths and their per-fire logs — right under the "Added cron schedule" confirmation, so you can see exactly what's scheduled without a separate lookup.
...
**Stats dashboard — Configs section.** The same resolution is also browsable at `branchdiff stats` → **Configs** (after Cron schedules): pick a launch directory (defaults to the repo the dashboard server is running in) and expand **Global** / **Launch dir** / each repo found under it to see its raw file content plus, per repo, the fully-resolved per-key table with an override highlight wherever that repo's own file won. Copy a file's path or content, or open it in your default editor, straight from the tree. The ⓘ button explains the same precedence rules as above — including the honest caveat that the launch-dir tier is wherever you point it, not a fixed parent folder, since branchdiff has no ancestor-directory config cascade.
...
- **`PATH`, `DISPLAY`, `DBUS_SESSION_BUS_ADDRESS`** are copied once, as a snapshot, straight into that schedule's crontab entry from the shell running `add` (it prints which ones it copied) — fine for these, since none of them rotate.
...
This covers the vars branchdiff itself needs to reach a forge or a desktop session. A custom `--exec` command's own env vars (an API key it reads itself, say) aren't handled by either mechanism above — add those as plain `VAR=value` lines directly in the crontab (`crontab -e`, at the top — they apply to every job in that file):
```
