...
- **`auto cron add` prints the crontab lines it wrote** — the exact start/end lines just written show up right under the confirmation message, so you can see exactly what's scheduled without a separate `crontab -l` lookup.
- **A "Configs" section on the Stats dashboard** — the same config-file resolution `branchdiff config` already prints, now browsable per launch directory, in the same four sections and order the CLI uses: `defaults`, `auto — launch tier`, one block per repo found under the launch dir (raw file content plus a fully-resolved per-key table showing exactly which tier won), and `auto — exec/tool`. Every key sits under its own section heading, so which tier a field belongs to is never a guess. Copy-path/copy-content/open-in-editor actions throughout. `branchdiff config` itself gains `--json` (machine-readable output) and `--dir <path>` (resolve against any directory, expanding to every repo directly under it when that directory isn't a repo itself) — both driven by the same resolver the dashboard uses, so the two can never disagree.
...
- **GUIDELINE.md is organized into four Parts** — Start here, View & explore, Review, and automation — each opening with a short orientation, led by a "What branchdiff does" intro that frames the whole tool as five capabilities (view any diff, comment with severity tags, review with any AI, automate PR review, sync with GitHub & Bitbucket).
...
- **Bitbucket auth setup now leads with the Atlassian API Token** — GUIDELINE.md and README.md recommend the Atlassian API Token (Option A) over the App Password, with the scoped-token steps spelled out (create the token **with scopes**, pick the Bitbucket app, grant **Read + Write** on repositories and pull requests, authenticate with your **email**). The App Password stays as Option B. The email-vs-username rule is unchanged: an Atlassian token uses your email, an App Password uses your Bitbucket username — mixing them still 401s.
